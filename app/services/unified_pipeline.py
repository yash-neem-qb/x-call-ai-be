...
class UnifiedCallPipeline:
    ...
    def __init__(self, call_sid: str, assistant_id: uuid.UUID, audio_callback: Callable[[str], Coroutine], text_callback: Optional[Callable[[str, str], Coroutine]] = None):
        """
    ...
    async def _handle_stt_complete(self, session_id: str, text: str):
        ...
        # Send user message to frontend if text callback is available
    ...
    async def _generate_llm_response(self, transcript: str, stt_end_time: float):
        ...
        try:
            async with self._processing_lock:
                ...
                # Send assistant message to frontend if text callback is available
                if self.text_callback and self.is_web_stream:
                    await self.text_callback("assistant", response)
    ...
    async def shutdown(self):
        ...
        try:
            ...
            logger.info(f"Pipeline shutdown completed for call {self.call_sid}")
...
class UnifiedPipelineManager:
    ...
    async def create_pipeline(self, call_sid: str, assistant_id: uuid.UUID, 
                            ...
                            text_callback: Optional[Callable[[str, str], Coroutine]] = None,
