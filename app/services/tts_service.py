...
class ElevenLabsTTSService:
    ...
    async def close_session(self, session_id: str) -> bool:
        """
        Close a TTS session and clear all audio buffers.
        ...
        try:
            ...
            # First, cancel any ongoing speech and clear audio buffers
            try:
                await self.cancel_speech(session_id)
                logger.info(f"Audio buffers cleared for session: {session_id}")
            except Exception as cancel_error:
                logger.warning(f"Error clearing audio buffers: {cancel_error}")
            ...
            logger.info(f"TTS session closed and audio cleared: {session_id}")
            return True
